...
def __build_print_pandoc_command() -> list:
    ...
    return ['timeout', '600', 'pandoc', engine, template, '--listings', '-V', 'documentclass=book', '-f', figures, '-o', output]
...
def __build_screen_pandoc_command() -> list:
    ...
    figures = 'markdown-implicit_figures'
    output = 'tmp_chapters.pdf'
    return ['timeout', '600', 'pandoc', engine, template, '--listings', '-V', 'documentclass=report', '-f', figures, '-o', output]
...
def __compile_print_opnening():
    xelatex_command = ['xelatex', '-output-directory', '.',
...
def __join_sections(output_name: str):
    ...
    ghostscript_command = ['gs', '-q', '-dNOPAUSE', '-dBATCH', '-sDEVICE=pdfwrite', output,
                           'opening.pdf', 'tmp_chapters.pdf']
...
if __name__ == "__main__":
    try:
        ...
        if sys.argv[1] == 'print':
            __create_print_pdf_from_stream(formatter.get_formatted_manuscript_stream_for_print_pdf())
        if sys.argv[1] == 'screen':
            __create_screen_pdf_from_stream(formatter.get_formatted_manuscript_stream_for_print_pdf())
        sys.exit(0)
