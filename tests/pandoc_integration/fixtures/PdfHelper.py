...
class PdfHelper:
    def __init__(self, raw_pdf: list[str]):
        self.raw_pdf = raw_pdf

    def get_links_from_pdf(self) -> list[str]:
        link_pattern = r"(.*[http][https]://.*)"
        return self._filter_list_by_pattern(self.raw_pdf, link_pattern)

    def get_page_numbers_of_headers_in_chapter(self, chapter_number: int) -> list[int]:
        page_numbers_preceding_pages_with_headers: list[str] = [
        ...
        page_numbers_with_headers: list[int] = [
            int(page_number) + 1
    ...
    def _get_links_from_manuscript_for_chapter(self, chapter_number: int) -> list[str]:
        link_pattern = r".*(http://.*)\)"
        with open(f"tests/pandoc_integration/fixtures/sample-manuscript/0{chapter_number}_chapter.md", "r") as f:
            lines = f.readlines()
