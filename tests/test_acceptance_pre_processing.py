...
class TestFormatter:
    def test_new_ebook_format(self):
        assert (epubFormatter.run("tests/fixtures/sample-manuscript").read().decode() == expected.epub_format)

    def test_new_pdf_print_format(self):
        assert (printPdfFormatter.run("tests/fixtures/sample-manuscript").read().decode() == expected.pdf_print)

    def test_new_pdf_screen_format(self):
        assert (screenPdfFormatter.run("tests/fixtures/sample-manuscript").read().decode() == expected.pdf_screen)
